...
# ----------------------------
log_info("Start fitting nearly concave quantile regression...")
lambda.vec <- c(0, 10 ^ seq(from = -2,to = 1, length.out = 10)) # Concave penalty
lambda2.vec <- 10 ^ seq(from = -4, to = 4, length.out = 10) # Smoothness penalty
res <- cv.q.concave(Y, X, Z, Yval, Xval, Zval, lambda.vec, lambda2.vec, tau0, sigma=SIGMA, verbose = 2)
hat.beta.concave <- res$hat.beta
hat.gamma.concave <- res$hat.gamma
hat.Ytest.concave <- Xtest.cat %*% hat.beta.concave + Ztest %*% hat.gamma.concave
...
lambda.vec <- c(0)
lambda2.vec <- 10 ^ seq(from = -4, to = 4, length.out = 10) # Smoothness penalty
res2 <- cv.q.concave(Y, X, Z, Yval, Xval, Zval, lambda.vec, lambda2.vec, tau0, sigma=SIGMA, verbose = 1)
...
cbind(beta.aggre, hat.beta.qr, hat.beta.concave, hat.beta.ridge, hat.beta.elastic, hat.beta.smooth,
hat.beta.spline, hat.beta.spline.ridge)

final[['beta.all']] <- cbind(beta.aggre, hat.beta.qr, hat.beta.concave, hat.beta.ridge, hat.beta.elastic, hat.beta.smooth, hat.beta.spline, hat.beta.spline.ridge)

resErr <- rep(0, 16)
resErr[1] <- sqrt(sum((hat.beta.qr - beta.aggre) ^ 2))
resErr[2] <- sqrt(sum((hat.beta.concave - beta.aggre) ^ 2))
...
resErr[10] <- pinball_loss(Ytest, hat.Ytest.concave, tau0)
...
cat(paste("Results-v02s/02", opt$model, "-",  opt$error,"/Res_", uu, ".RDS", sep = ""))
saveRDS(final, file = paste("Results-v02s/02", opt$model, "-",  opt$error,"/Res_", uu, ".RDS", sep = ""))
